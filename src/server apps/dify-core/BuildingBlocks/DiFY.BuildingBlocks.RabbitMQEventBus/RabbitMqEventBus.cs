...
namespace DiFY.BuildingBlocks.EventBus
{
    public class RabbitMqEventBus : IEventsBus, IDisposable
    {
        ...
        public void Publish(IntegrationEvent @event)
        {
            ...
            _logger.Information("Creating RabbitMQ channel to publish event: " +
                                "{EventId} ({EventName})", @event.Id, eventName);
            ...
            if (_subsManager.HasSubscriptionsForEvent(eventName))
            {
                ...
                foreach (var integrationEventHandler in integrationEventHandlers)
                {
                    ...
                    var @event = JsonSerializer.Deserialize(
                        message, eventType, new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
                    var concreteType = typeof(IIntegrationEventHandler<>).MakeGenericType(eventType);
                    await Task.Yield();
                    await ((Task)concreteType.GetMethod("Handle")?.Invoke(
                        integrationEventHandler, new object[] { @event }))!;
                }
            }
            else
            {
